...
### v1.6.0 — Classic formats are now faithful legacy replicas
`Neotec Tax Invoice (Classic)` / `Neotec Delivery Note (Classic)` render from
`print/neotec_document_classic.html`: plain black-bordered layout matching the legacy
sheet block-for-block (header with logos + document meta + package grid, party band with
GST/e-invoice/contact columns, legacy item-table column order per doctype, charges band
with SACs, amount-in-words + Invoice Total row, payment-terms strip, reverse-charge +
balances box, remarks/bank/terms/signatory band, HSN summary including charge SACs).
All Neotec Print Settings toggles still apply.
