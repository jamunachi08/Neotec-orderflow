...
PRINT_FORMATS = {
    ...
    "Neotec Delivery Note (Classic)": ("Delivery Note", "neotec_document_classic.html",
                             '{% set nof_kind = "dn" %}{% set nof_variant = "classic" %}\n'),
    "Neotec Tax Invoice (Classic)": ("Sales Invoice", "neotec_document_classic.html",
...
def ensure_all():
    ...
    try:
        if frappe.db.exists("DocType", "Neotec Print Settings"):
...
def sync_print_formats():
    """Manual re-sync from the Neotec Print Settings button. Returns a report."""
    frappe.only_for(("System Manager",))
    report = ensure_print_formats()
    frappe.db.commit()
    return report
...
def ensure_print_formats():
    ...
    for name, (doctype, filename, prefix) in PRINT_FORMATS.items():
        try:
            _sync_one_print_format(name, doctype, filename, prefix)
            report.append(f"OK: {name} → {doctype}")
        except Exception:
            frappe.log_error(frappe.get_traceback(), f"Neotec Print Format sync: {name}")
            report.append(f"FAILED: {name} (see Error Log)")
        frappe.db.commit()
    return report


def _sync_one_print_format(name, doctype, filename, prefix):
    if True:
