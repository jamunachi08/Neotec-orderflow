...
def ensure_all():
    ...
    try:
        ...
        ps.db_update()
        apply_default_print_formats(bool(ps.set_as_default))
    except Exception:
...
def apply_default_print_formats(enable: bool):
    """Set / clear the Neotec formats as the default for DN & SI."""
    from frappe.custom.doctype.property_setter.property_setter import make_property_setter
    for doctype, pf in DEFAULTABLE.items():
        if enable:
            make_property_setter(doctype, None, "default_print_format", pf, "Data",
