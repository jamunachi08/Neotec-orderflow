...
def _tax_mode(s, tax_totals, items):
    ...
    if mode == "rates":
        cols = _applicable_cols(tax_totals, items) or ["cgst", "sgst", "igst"]
        return {"show": True, "cols": cols, "rates_only": True}
...
def nof_print_context(doc, kind="si"):
    ...
    ctx = frappe._dict(
        s=s, kind=kind, currency=currency,
        colors=frappe._dict(
        ...
        min_rows=cint(s.get("min_item_rows")) if s.get("min_item_rows") is not None else 6,
        flags=frappe._dict(
