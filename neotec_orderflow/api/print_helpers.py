...
def _safe_get_value(doctype, name, fields):
    """get_value that only asks for columns the DocType actually has —
    optional fields (e.g. India Compliance's Company.gstin) simply come
    back as None instead of throwing 1054 Unknown column."""
    meta = frappe.get_meta(doctype)
    present = [f for f in fields if meta.has_field(f) or f == "name"]
    out = frappe._dict({f: None for f in fields})
    if present and name:
        got = frappe.db.get_value(doctype, name, present, as_dict=True)
        if got:
            out.update(got)
    return out
...
def _company_logo(doc, s):
    ...
    c = _safe_get_value("Company", doc.company, ["company_logo", "default_letter_head"])
    if c.company_logo:
        return c.company_logo
    # fallback: the letter head image, if any
    lh = doc.get("letter_head") or c.default_letter_head
    if lh:
        img = _safe_get_value("Letter Head", lh, ["image"]).image
        if img:
            return img
    return None
...
def _brand_logo(doc):
    ...
    if len(brands) != 1:
        return None, None
    brand = brands.pop()
    img = _safe_get_value("Brand", brand, ["image"]).image
    return brand, img
...
def _company_info(doc):
    c = _safe_get_value(
        "Company", doc.company,
        ["company_name", "gstin", "tax_id", "phone_no", "email", "website"],
    )
    gstin = doc.get("company_gstin") or c.get("gstin") or c.get("tax_id")
    return frappe._dict(
...
def _item_rows(doc, s):
    ...
    for i in doc.get("items") or []:
        ...
        rows.append(frappe._dict(
            ...
            unit_price=plr, rate=rate, gross=gross, disc_pct=disc_pct, disc_amt=max(disc_amt, 0),
            taxable=taxable,
            cgst_rate=cg_r, cgst_amount=cg_a, sgst_rate=sg_r, sgst_amount=sg_a,
            igst_rate=ig_r, igst_amount=ig_a, line_total=line_total,
            finish=i.get("custom_finish") or i.get("finish"),
...
def _charges_band(doc, s, tax_totals, items, tax_cols):
    ...
    default_rows = "Freight|996511\nInsurance|997139\nPacking and Forwarding Expenses|996713"
    wanted = []
    for ln in (s.get("classic_charge_rows") or default_rows).splitlines():
        ln = ln.strip()
        if not ln:
            continue
...
def nof_print_context(doc, kind="si", variant="modern"):
    ...
    if classic:
        ...
        s.tax_display = s.get("classic_si_tax_display" if kind == "si" else "classic_dn_tax_display") \
            or "All — CGST, SGST & IGST"
        s.show_company_logo = cint(s.get("classic_show_company_logo", 1)) \
            if s.get("classic_show_company_logo") is not None else 1
        s.show_brand_logo = cint(s.get("classic_show_brand_logo", 1)) \
            if s.get("classic_show_brand_logo") is not None else 1
        s.show_discount_columns = cint(s.get("classic_show_discount_columns", 1)) \
            if s.get("classic_show_discount_columns") is not None else 1
    ...
    ctx = frappe._dict(
        ...
        balance=balance,
        fmt=lambda v: fmt_money(flt(v), precision=2),   # plain 1,058.00 — legacy sheet has no currency symbol
        ...
        discount_note=s.get("discount_note") if s.get("discount_note") is not None else
            "Discounts are applicable on Unit price and GST is applicable on Assessable value.",
